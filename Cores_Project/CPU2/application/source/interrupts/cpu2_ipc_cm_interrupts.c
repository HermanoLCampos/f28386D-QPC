...
//    BSP_BKPT;
    ...
    QACTIVE_POST_FROM_ISR( p_ao_communication , &im_evt_ipc_receive_msg[OC_IPC_CPU2_CM_ID].super , &xHigherPriorityTaskWoken , (void *) 0 );
...
//    BSP_BKPT;
    ...
    IPC_readCommand( IPC_CPU2_L_CM_R , IPC_RESET_FLAG , IPC_ADDR_CORRECTION_DISABLE, &command, &addr, &data);

    switch (command) {
        case OC_IPC_CMD_REMOTE_RESET:
            QACTIVE_POST_FROM_ISR( p_ao_communication , &im_evt_ipc_remote_reset[OC_IPC_CPU2_CM_ID].super , &xHigherPriorityTaskWoken , (void *) 0 );
            break;
        case OC_IPC_CMD_RESET_COMPLETE:
            QACTIVE_POST_FROM_ISR( p_ao_communication , &im_evt_ipc_reset_complete[OC_IPC_CPU2_CM_ID].super , &xHigherPriorityTaskWoken , (void *) 0 );
            break;
        default:
            break;
    }
